...
class EquationGroup(Node):
    ...
    def __set_name__(self, cls: Node, name: str):
        for eq in getattr(self, "equations", []):
...
class Container(Node):
    ...
    def __setitem__(self, index: int, value: Node):
        raise AttributeError("Containers items cannot be modified once added")

    def __delitem__(self, index: int):
        raise AttributeError("Items cannot be deleted from containers")

    def __len__(self):
        return len(self.nodes)

    def __repr__(self):
        return f"Container: {self.nodes}"

    def __set_name__(self, cls: Node, name: str):
        for index, node in enumerate(self.nodes):
            node.__set_name__(
                cls, f"name_{index}"
            )  # TODO: should the System or the Container be the parent?
        super().__set_name__(cls, name)

    def _yield(self, type, /, *, exclude=..., recursive=True):
        if recursive:
            for node in self.nodes:
                yield from node._yield(type, exclude=exclude, recursive=recursive)
        else:
...
#     def __init__(
#         self,
#         objects: Iterable[Initial | None] | None = None,
#         lazy: bool = False,
#     ):
#         super().__init__()
#         if lazy:
#             if objects is not None:
#                 raise ValueError("Lazily create Containers cannot be passed objects")
#             self.nodes = []
#         else:
#             self.nodes = objects
#         self.is_lazy = lazy
...
#         if not self.is_lazy:
#             raise AttributeError("Items can only be added to lazy Containers")

#         if self.name != "":
#             value.__set_name__(self.parent, f"{self.name}_{len(self.nodes)}")
...
#     def __set_name__(self, cls: Node, name: str):
#         for index, node in enumerate(self.nodes):
#             node.__set_name__(cls, f"name_{index}")
#         super().__set_name__(cls, name)
