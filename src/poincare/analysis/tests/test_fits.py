...
def test_fit_without_units():
    ...
    with pytest.raises(pint.PintError):
        fit = fit_result(sim=sim, results=target, p0 = {Oscillator.omega: omega_real*1.1, Oscillator.x: (None, -np.inf*ureg.m, 10)}, )

    fit = fit_result(sim=sim, results=target, p0 = {Oscillator.omega: omega_real*1.1, Oscillator.x: (None, -np.inf, 10)}, )
...
def test_fit_with_units():
    ...
    u_target = make_target(results={"x": u_target_x, UnitsOscillator.vx: u_target_v}, save_at=u_t)
    u_fit = fit_result(sim=usim, results=u_target, p0 = {UnitsOscillator.omega: u_omega_real*1.1, UnitsOscillator.x: (None, -np.inf*ureg.m, 1000*ureg.cm)})

    rtol = 0.025
    assert np.isclose(u_fit[UnitsOscillator.x].to(ureg.m).magnitude, UnitsOscillator.x.initial.magnitude, rtol=rtol)
    assert np.isclose(u_fit[UnitsOscillator.omega].to(1/ureg.s).magnitude, u_omega_real.to(1/ureg.s).magnitude, rtol=rtol) 
