...
class Fitter:
    def __init__(self,     
        sim: Simulator,
        results: xr.Dataset,
        simulation_function_generator: Callable[[Fitter], Callable[[list], xr.Dataset]],
        units_handler: type[UnitsHandler] = UnitsHandler, 
        p0: Mapping[Components, Initial | tuple[Initial | None, Initial, Initial] | None] = {},  # read only
        scale: Mapping[Components | str, Number] | None = None,
        ):
        self.fit_variables = list(results.keys())
        if p0:
            self.fit_parameters = list(p0.keys())
        else:
            self.fit_parameters = list(param for param in sim.model.parameters.values)
        self.clean_p0, bounds = parse_p0(p0) 
        if scale is not None:
            self.scale = np.array([[scale.get(var, 1)] for var in self.fit_variables])
        else:
            self.scale = np.array([[1] for var in self.fit_variables])
        self.units = units_handler(self.fit_variables, self.fit_parameters, results, self.clean_p0)
        self.bounds = tuple(np.array([self.units.dequantify_bounds(bounds=bounds, param=param, i = i) for param in self.fit_parameters]) for i in range(2))
        self.y0 = np.array(
            [self.units.dequantify_result(results[var], var) for var in self.fit_variables]
        )
        self.simulation_function = simulation_function_generator(self)

    def solve(self, **kwargs):
        def f(x):
                x = [
                    self.units.quantify_parameter(x[i], param)
                    for i, param in enumerate(self.fit_parameters)
                ]
                result = self.simulation_function(x)
                y = np.array(
                    [self.units.dequantify_result(result[str(var)], var) for var in self.fit_variables]
                )
                res = (y - self.y0) * self.scale  # TODO: is * scale or / scale more intuitive?
                return res.ravel()
        
        x0 = np.array(
            [
                self.units.dequantify_parameter(
                    self.clean_p0.get(param, get_default_or_inital(param)), param
                )
                for param in self.fit_parameters
            ]
        )
        solution = least_squares(f, x0, bounds = self.bounds, **kwargs)
        formatted_solution = {
            var: self.units.quantify_parameter(solution.x[i], var)
            for i, var in enumerate(self.fit_parameters)
        }
        return formatted_solution
